...
@dataclass
class SkipBertOutput(ModelOutput):
...
class BertModelWithSkipDecoding(BertPreTrainedModel):
    ...
    ) -> Union[Tuple[torch.Tensor], BaseModelOutputWithPoolingAndCrossAttentions, SkipBertOutput]:
    ...
    def _forward_with_skip_decoding(
        self,
    ...
    ) -> Union[Tuple[torch.Tensor], SkipBertOutput]:
        ...
        for i, layer_module in enumerate(self.encoder.layer):
            ...
            if output_attentions:
                ...
                attention[active_samples] = layer_outputs[1]
...
class BertForSequenceClassificationWithSkipDecoding(BertPreTrainedModel):
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        skip_decoding: bool = False,
    ) -> Union[Tuple[torch.Tensor], SkipBertSequenceClassifierOutput]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the sequence classification/regression loss. Indices should be in `[0, ...,
            config.num_labels - 1]`. If `config.num_labels == 1` a regression loss is computed (Mean-Square loss), If
